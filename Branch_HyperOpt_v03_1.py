...
def f_nn(params):
    dataset, output_class = V_feeder_noDC[0], Branch_Output_sorted[0]
    ...
    try:
        outfile = open(directory + r'\branch_id_v03_1.csv', 'wb')
        df.to_csv(outfile)
        outfile.close()
...
trainX, testX, trainY, testY = shuffle_dataset(V_feeder_noDC[0], Branch_Output_sorted[0])
